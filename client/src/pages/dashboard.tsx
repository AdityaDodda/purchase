...
export default function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState('my'); // 'my' or 'approver'

  const { data: stats } = useQuery({
    queryKey: ["/api/dashboard/stats"],
  });

  const { data: user } = useUserQuery({ queryKey: ["/api/auth/user"] });

  // Fetch requests for the current tab
  const queryParams = activeTab === 'approver' && user
    ? { currentApproverId: user.id, status: 'pending' }
    : activeView === 'all'
      ? {}
      : { status: activeView };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-20">
        ...
        <div className="flex space-x-2 mb-4">
          <Button
            ...
            className={activeTab === 'my' ? "bg-[hsl(207,90%,54%)] hover:bg-[hsl(211,100%,29%)]" : ""}
          ...
          {user?.role === 'approver' && (
            <Button
              variant={activeTab === 'approver' ? 'default' : 'outline'}
              className={activeTab === 'approver' ? "bg-green-600 hover:bg-green-700 text-white" : ""}
